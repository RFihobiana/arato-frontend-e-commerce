...
export const updateCategorie = async (id, nomCategorie) => {
    try {
        const res = await api.put(`${CATEGORIE_URL}/${id}`, { nomCategorie }, getConfig());
        return res.data;
    } catch (error) {
        console.error("Erreur mise à jour catégorie: ", error.response?.data || error.message);
        throw error;
    }
};
